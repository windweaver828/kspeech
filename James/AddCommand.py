...
command_file = os.path.expanduser("~/Programs/kspeech/Commands.py")
writefile = os.path.expanduser("~/.config/kspeech/commands.conf")
    ...
    def Config(self, command, function, args):
        with open(command_file, 'a') as f:
            ...
            if not len(command) <2:
                f.write(wline)
        Popen(os.path.expanduser('~/.config/kspeech/language_updater.sh'), shell=True)
